...
log = logging.getLogger("term_bridge.queue")
...
class ControllerQueue:
    def __init__(
        ...
        *,
        ...
        on_complete: Callable[[ControllerJob, BaseException | None], None] | None = None,
    ) -> None:
        if max_attempts < 1:
    ...
    def start(self) -> None:
        with self._condition:
            ...
            self._thread = threading.Thread(
                ...
                name=f"term_bridge_worker_{self.label}",
                daemon=False,
            )
            self._thread.start()
    ...
    def health_snapshot(self) -> dict[str, object]:
        with self._condition:
            return {
                ...
                "active_kind": self._current_kind,
                "pending_jobs": len(self._items),
                "pending_sets": sum(1 for job in self._items if job.kind == "set"),
            ...
            try:
                ...
                try:
                    ...
                    if self._on_complete is not None:
                        try:
                            self._on_complete(job, job.error)
                        except Exception:  # noqa: BLE001 - status reporting must not kill the worker
                            log.exception("Controller completion callback failed for %s", self.label)
                    job.done.set()
