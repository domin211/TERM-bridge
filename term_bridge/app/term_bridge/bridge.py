...
from .config import (
    ...
    MQTT_DISCOVERY_CLEANUP_DELAY,
    MQTT_KEEPALIVE,
    MQTT_LEGACY_ZONE_LIMIT,
    MQTT_RETRY_SECONDS,
...
log = logging.getLogger("term_bridge.bridge")
...
class TERMBridge:
    def __init__(self, options: Options) -> None:
        ...
        self.last_successful_sync: dict[str, dict[str, float | None]] = {
            c.key: {"temps": None, "regs": None} for c in options.controllers
        }
        self.controller_online: dict[str, bool] = {c.key: False for c in options.controllers}
        self.controller_last_error: dict[str, str | None] = {c.key: None for c in options.controllers}
        self.last_health_payload: str | None = None
        self.client_id = options.mqtt_client_id or get_or_create_client_id(options.instance_id_path)
        self.discovery_registry = DiscoveryRegistry(options.discovery_registry_path)
        ...
        self.queues: dict[str, ControllerQueue] = {
            c.key: ControllerQueue(
                ...
                on_complete=self._make_completion_callback(c),
            )
        ...
        if options.mqtt_tls:
            self.mqtt.tls_set(
                ca_certs=options.mqtt_tls_ca or None,
                certfile=options.mqtt_tls_cert or None,
                keyfile=options.mqtt_tls_key or None,
    ...
    def _make_executor(self, controller: Controller):
        def execute(job: ControllerJob) -> None:
            ...
            if job.kind == "temps":
                ...
                self.last_successful_sync[controller.key]["temps"] = time.time()
                ...
                self.last_successful_sync[controller.key]["regs"] = time.time()
    ...
    def _make_completion_callback(self, controller: Controller):
        def complete(job: ControllerJob, error: BaseException | None) -> None:
            if job.kind not in ("temps", "regs"):
                return
            self.controller_last_error[controller.key] = str(error) if error is not None else None
            self._set_controller_online(controller, error is None)
    ...
    def _apply_regs(self, controller: Controller, lines: list[str]) -> None:
        ...
        previously_published = self.discovery_registry.previous(controller.key)
        for unique_id in previously_published - valid_unique_ids:
            if unique_id not in stale:
                self._remove_zone(unique_id)
        self.discovery_registry.replace(controller.key, valid_unique_ids)
    ...
    def _publish_empty_retained(self, topics: set[str]) -> object | None:
        last_result = None
        for topic in sorted(topics):
            last_result = self.mqtt.publish(topic, "", qos=1, retain=True)
        return last_result

    def _legacy_zone_topics(self, unique_id: str) -> set[str]:
        return {
            self.topics.discovery(unique_id),
            self.topics.state(unique_id),
            self.topics.command(unique_id),
            f"{self.options.discovery_prefix}/climate/hmpd_{unique_id}/config",
            f"hmpd/{unique_id}/state",
            f"hmpd/{unique_id}/set_target",
        }

    def _run_naming_migration(self) -> None:
        marker_value = self.options.naming_migration_marker_path
        if not marker_value:
            return
        ...
        log.warning("Running one-time MQTT discovery cleanup for corrected entity names")
        topics: set[str] = set()
        for controller in self.controllers:
            topics.add(self.topics.controller_status_discovery(controller.key))
            limit = max(MQTT_LEGACY_ZONE_LIMIT, controller.expected_regs)
            for zone_index in range(limit):
                topics.update(self._legacy_zone_topics(zone_unique_id(controller.key, zone_index)))

        last_result = self._publish_empty_retained(topics)
        wait_for_publish = getattr(last_result, "wait_for_publish", None)
        if callable(wait_for_publish):
            wait_for_publish(timeout=10)
        time.sleep(MQTT_DISCOVERY_CLEANUP_DELAY)

        for controller in self.controllers:
            self._publish_controller_discovery(controller)
            self._set_controller_online(controller, self.controller_online[controller.key], force=True)

        try:
            marker.parent.mkdir(parents=True, exist_ok=True)
            marker.write_text("completed\n", encoding="ascii")
        except OSError as exc:
            log.error("Could not persist naming migration marker %s: %s", marker, exc)
            return
        log.info("One-time MQTT discovery cleanup complete (%s retained topics cleared)", len(topics))
    ...
    def _publish_controller_discovery(self, controller: Controller) -> None:
        payload = json.dumps(self.topics.controller_status_payload(controller), ensure_ascii=False, sort_keys=True)
        self.mqtt.publish(
            self.topics.controller_status_discovery(controller.key),
            payload,
            qos=1,
            retain=True,
        )

    def _set_controller_online(self, controller: Controller, online: bool, *, force: bool = False) -> None:
        changed = self.controller_online[controller.key] != online
        self.controller_online[controller.key] = online
        if self.mqtt_connected and (changed or force):
            self.mqtt.publish(
                self.topics.controller_status(controller.key),
    ...
    def health_payload(self) -> dict[str, object]:
        ...
        for controller in self.controllers:
            sync = self.last_successful_sync[controller.key]
            controllers[controller.key] = {
                "name": controller.name,
                "device": controller.dev,
                "last_successful_sync": {
                    "temps": self._timestamp(sync["temps"]),
                    "regs": self._timestamp(sync["regs"]),
                },
                "online": self.controller_online[controller.key],
                "last_error": self.controller_last_error[controller.key],
                "queue": self.queues[controller.key].health_snapshot(),
            }
        return {
            "status": "online" if self.mqtt_connected else "offline",
            "mqtt_connected": self.mqtt_connected,
            "mqtt_client_id": self.client_id,
            "uptime_seconds": max(0, int(time.time() - self.started_at)),
            "zones": len(self.zones),
            "controllers": controllers,
            "generated_at": self._timestamp(time.time()),
        }

    def _publish_health(self, *, force: bool = False) -> None:
        if not self.mqtt_connected:
            return
        payload = json.dumps(self.health_payload(), ensure_ascii=False, sort_keys=True)
        if not force and payload == self.last_health_payload:
            return
        self.mqtt.publish(self.topics.bridge_health(), payload, qos=1, retain=True)
    ...
    def _mqtt_connect_loop(self) -> bool:
        while not self.shutdown_event.is_set():
            try:
                ...
                log.error(
                    "MQTT host %r could not be resolved (%s). Verify that the broker add-on is installed, "
                    "running, and that mqtt_host matches its service hostname.",
    ...
    def start(self) -> None:
        try:
            ...
            log.info("Shutting down TERM Bridge")
            ...
            for controller in self.controllers:
                self._set_controller_online(controller, False, force=True)
            try:
                self.mqtt.loop_stop()
                self.mqtt.disconnect()
            except Exception:
                pass
            log.info("TERM Bridge shutdown complete")
