...
from term_bridge.config import DEBUG_LOG_BACKUP_COUNT, DEBUG_LOG_FILE, DEBUG_LOG_MAX_BYTES, load_options
...
def configure_logging(debug: bool) -> None:
    ...
    try:
        ...
        logging.getLogger("term_bridge").info(
        ...
        logging.getLogger("term_bridge").warning("Could not set up debug file handler: %s", exc)


def main() -> None:
    options = load_options()
    configure_logging(options.debug)
    log = logging.getLogger("term_bridge")

    try:
        TERMBridge(options).start()
